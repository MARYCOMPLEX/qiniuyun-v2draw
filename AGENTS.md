...
**未经用户明确要求, 禁止改 UI** — 包括但不限于:

- 删除 / 移动 / 重命名页面已有组件
- 改变布局栅格 / 列宽 / 间距
- 删除已有的视觉元素 (音量条 / 状态徽章 / 装饰)
- 把现有面板拆成对话框 / 反向操作
- 改动配色 / 字体 / 主题 token

**允许** (无需用户确认):
- 在不改其他元素的前提下, 修复确认的 UI bug (如 hydration mismatch / 错位)
- 新增可选 prop, 默认值保持原行为
- typo / 错别字 / 拼写

**判定规则**: 重写 / 替换组件时如果新版本字段比旧版本少 (例: TelemetryHUD → AgentConversationPanel
丢了音量条), 必须在 PR 描述里**显式声明**"删除了 X 元素", 否则视为意外丢失需要补回。

**Why**: 用户在 UI 上投入了大量审美决策, 静默改动 = 摧毁工作。
2026-06-14 案例: AgentConversationPanel 重写时把 TelemetryHUD 顶部 24 格音量条柱状图丢了,
用户两次反馈"右侧上面的组件又没有了"。每一次没明确要求的 UI 改动都是一次返工。
